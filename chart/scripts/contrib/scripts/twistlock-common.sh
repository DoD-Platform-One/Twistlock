...
if [ "$(type -t callapi)" != "function" ]; then
  ...
  echo "Twistlock URL: $TWISTLOCK_URL"
  ...
  callapi() {
    ...
    if [[ "$request" == "PATCH" ]]; then
      local args=("-k"  "-s" "-X" "$request" "-H" "Content-Type: application/json")
    else
    ...
    # Send request
    local result; result=$(curl "${args[@]}" https://"$TWISTLOCK_URL/api/v1/$endpoint")
    ...
    declare -i STATUS=$(printf "%s" "$result" | tail -c 3)
  ...
  #######################################
  # Prints URL-encoded version of a string
  # Arguments:
  #   String to encode
  # Outputs:
  #   URL-encoded string
  ...
  #######################################
  # Prints combined JSON string
  # Arguments:
  #   Two input JSON strings
  #   Values in second object override
  #   those in the first
  # Outputs:
  #   Combined JSON string
  #######################################
  mergejson() { echo "${1}${2}" | jq -n 'reduce inputs as $item ({}; . *= $item)' ; }
